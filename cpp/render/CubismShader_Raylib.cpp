...
// Mesh buffers must fit both vertex and index streams for all drawable meshes.
const int MAX_VERTEX_COUNT = 65535;
const int MAX_INDEX_COUNT = 65535;
...
void RenderBufferManager::CreateBuffer(int shaderId, const CubismShaderSet*& shaderSet, CubismRenderBufferSet*& bufferSet)
{
	...
	else
	{
		...
		rlSetVertexAttribute(shaderSet->attribPosition, 2, RL_FLOAT, false, sizeof(csmFloat32) * 2, 0);
		...
		rlSetVertexAttribute(shaderSet->attribTexCoord, 2, RL_FLOAT, false, sizeof(csmFloat32) * 2, 0);

		buffer.vertexBufferElementId = rlLoadVertexBufferElement(NULL, sizeof(unsigned short) * MAX_INDEX_COUNT, true);
