...
namespace
{
    csmByte *CreateBuffer(const csmChar *path, csmSizeInt *size)
    {
        ...
        return LoadFileData(path, reinterpret_cast<int *>(size));
    }
...
void LAppModel::Initialize(const csmChar *dir, const csmChar *fileName)
{
    ...
    this->CubismUserModel::CreateRenderer(GetScreenWidth(), GetScreenHeight(), 1);
    if (GetRenderer<CubismRenderer_Raylib>() != nullptr)
    {
        LAppPal::PrintLog("[APP] Raylib renderer created successfully.");
    }
    SetupTextures();
}
...
void LAppModel::SetupModel(ICubismModelSetting *setting)
{
    ...
    if (strcmp(_modelSetting->GetModelFileName(), "") != 0)
    {
        ...
        {
            LAppPal::PrintLog("[APP] ERROR: _model is NULL after LoadModel!");
        }
        else
        {
            LAppPal::PrintLog("[APP] SUCCESS: _model loaded.");
        }
    }
    ...
    if (_modelSetting->GetExpressionCount() > 0)
    {
        ...
        for (csmInt32 i = 0; i < count; i++)
        {
            ...
            if (motion != NULL)
            {
                // Keep expression switching snappy for realtime face controls.
...
Csm::csmInt32 LAppModel::GetExpressionCount() const
{
    return _expressionNames.GetSize();
}

const Csm::csmChar *LAppModel::GetExpressionName(Csm::csmInt32 index) const
{
    if (index < 0 || index >= _expressionNames.GetSize())
    {
        return "";
    }
    return _expressionNames[index].GetRawString();
}

Csm::csmBool LAppModel::IsExpressionPlaying() const
{
    return _expressionManager != NULL && !_expressionManager->IsFinished();
}
...
CubismMotionQueueEntryHandle LAppModel::StartMotion(const csmChar *group, csmInt32 no, csmInt32 priority, ACubismMotion::FinishedMotionCallback onFinishedMotionHandler)
{
    ...
    {
        if (_debugMode && priority > PriorityIdle)
        {
...
void LAppModel::DoDraw()
{
    ...
    GetRenderer<CubismRenderer_Raylib>()->DoDrawModel1();
}
...
void LAppModel::Draw(CubismMatrix44 &matrix)
{
    ...
    CubismMatrix44 finalMatrix;
    finalMatrix.LoadIdentity();
    
    finalMatrix.MultiplyByMatrix(&matrix);
    
    finalMatrix.MultiplyByMatrix(_modelMatrix);

    GetRenderer<CubismRenderer_Raylib>()->SetMvpMatrix(&finalMatrix);
    DoDraw();
}
...
{
    if (expressionID == NULL || expressionID[0] == '\0')
    {
        LAppPal::PrintLog("[APP][expr] SetExpression called with empty expression id");
        return;
    }

    if (_expressionManager == NULL)
    {
        LAppPal::PrintLog("[APP][expr] SetExpression('%s') failed: _expressionManager is NULL", expressionID);
        return;
    }
    ...
    {
        LAppPal::PrintLog("[APP][expr] motion not found for '%s'", expressionID);
        csmString names;
        for (csmInt32 i = 0; i < _expressionNames.GetSize(); ++i)
        {
            if (i > 0)
            {
                names += ", ";
            }
            names += _expressionNames[i];
        }
        LAppPal::PrintLog("[APP][expr] available names: %s", names.GetRawString());
    }
}

void LAppModel::StopExpression()
{
    if (_expressionManager != NULL)
    {
        _expressionManager->StopAllMotions();
...
void LAppModel::SetupTextures()
{
    ...
    GetRenderer<CubismRenderer_Raylib>()->IsPremultipliedAlpha(true);
#else
